...
namespace ae2f {
    ...
    struct errGlobState {
        ae2f_err_t code;
        constexpr errGlobState(ae2f_err_t a) noexcept;
        ...
        static constexpr errGlobState errGlobStateWithThrow(ae2f_err_t a);
        ...
        constexpr const char* peek() const noexcept {
            if(code & ae2f_errGlob_IMP_NOT_FOUND) {
                return "Failed to find the function on preprocessor which is callable for some reason No operation has beed done.";
            }
            ...
            if(code & ae2f_errGlob_ALLOC_FAILED) {
                return "stdlib allocating functions (malloc, calloc, realloc) has been failed.";
            }

            if(code & ae2f_errGlob_WRONG_OPERATION) {
                return "Found that parameter sent by programmer is invalid. The operation may have been ceased while the middle.";
            }

            if(code & ae2f_errGlob_NFOUND) {
                return "Found some errors, but not by parameters. The operation has failed.";
            }
    ...
    constexpr errGlobState::errGlobState(ae2f_err_t a) noexcept : code(a) {}
    ...
    class errGlobThrown : public std::exception {
        ...
        inline errGlobThrown(const ae2f_err_t c)
        noexcept : code(c), msg(code.msgall()) {}
    ...
    constexpr errGlobState errGlobState::errGlobStateWithThrow(ae2f_err_t a) {
        if(ae2f_errGlobNormalised(a)) throw errGlobThrown(a);
        return a;
    }
}
#include "Pack/End.h"
